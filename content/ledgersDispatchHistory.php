...
// Fetch all ledger dispatch records
$query = "
    SELECT 
        ls.*,
    ...
    GROUP BY ls.saleID
    ORDER BY ls.saleDate DESC, ls.createdAt DESC
";
...
<div class="history-container">
    <div class="history-header">
        <h2>📚 Ledgers Dispatch History</h2>
        <p style="margin: 0; opacity: 0.9;">Complete record of all dispatches to ledgers</p>
    </div>
    ...
    <div class="table-card">
        <input type="text" id="searchHistory" class="search-box" placeholder="🔍 Search by ledger name, CNIC, or Sale ID...">
...
<script>
    ...
    function viewDispatch(saleID) {
        ...
        $.ajax({
            url: 'backend/getLedgerDispatchDetails.php',
            type: 'GET',
            data: { saleID: saleID },
            dataType: 'json',
    ...
    function displayDispatchDetails(data) {
        ...
        const html = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    <h4 style="margin: 0 0 15px 0; color: #11998e;">Sale Information</h4>
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">${data.sale.dueDate && data.sale.dueDate !== '0000-00-00' ? new Date(data.sale.dueDate).toLocaleDateString('en-GB') : 'N/A'}</span>
                    </div>
                </div>
                <div>
                    <h4 style="margin: 0 0 15px 0; color: #11998e;">Ledger Information</h4>
                    <div class="detail-row">
                        <span class="detail-label">Name:</span>
                        <span class="detail-value">${data.sale.ledgerName}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">CNIC:</span>
                        <span class="detail-value">${data.sale.ledgerCNIC || 'N/A'}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Contact:</span>
                        <span class="detail-value">${data.sale.ledgerContact || 'N/A'}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Address:</span>
                        <span class="detail-value">${data.sale.ledgerAddress || 'N/A'}</span>
                    </div>
                </div>
            </div>
            
            <h4 style="margin: 20px 0 10px 0; color: #11998e;">Items</h4>
            <table class="history-table">
                <thead>
                    <tr>
                        <th>#</th>
            ...
            <div style="background: #f0fdf9; padding: 20px; border-radius: 8px; margin-top: 20px;">
                <div class="detail-row">
                ...
                <div class="detail-row">
                    <span class="detail-label" style="font-size: 18px; color: #11998e;">Grand Total:</span>
                    <span class="detail-value" style="font-size: 18px; font-weight: bold; color: #11998e;">RS ${parseFloat(data.sale.grandTotal).toFixed(2)}</span>
                </div>
                <div class="detail-row">
            ...
            <h4 style="margin: 20px 0 10px 0; color: #11998e;">💳 Payment History</h4>
            <table class="history-table">
                <thead>
                    <tr>
                        <th>#</th>
