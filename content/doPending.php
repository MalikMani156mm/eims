...
// Fetch pending and partially paid dispatches (only include approved sales)
$query = "
    SELECT 
        ds.*,
        do.DO_Name,
        ...
        DATEDIFF(ds.dueDate, CURDATE()) as daysRemaining,
        CASE 
            WHEN ds.dueDate IS NULL THEN 'no_due'
            WHEN DATEDIFF(ds.dueDate, CURDATE()) < 0 THEN 'overdue'
            WHEN DATEDIFF(ds.dueDate, CURDATE()) <= 7 THEN 'urgent'
            ELSE 'normal'
    ...
    WHERE ds.paymentStatus IN ('pending', 'partial') AND (ds.approvedByAdmin IS NOT NULL AND ds.approvedBySuperAdmin IS NOT NULL)" . $regionFilter . "
    GROUP BY ds.saleID
    ORDER BY 
        CASE 
            WHEN ds.dueDate IS NULL THEN 3
            WHEN DATEDIFF(ds.dueDate, CURDATE()) < 0 THEN 1
            WHEN DATEDIFF(ds.dueDate, CURDATE()) <= 7 THEN 2
            ELSE 3
        END,
        ds.dueDate ASC
";
...
<div id="paymentModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5); overflow:auto;">
    <div style="background:white; margin:5% auto; padding:0; width:90%; max-width:500px; border-radius:12px; box-shadow:0 10px 40px rgba(0,0,0,0.3);">
        <div style="background:linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color:white; padding:20px 25px; border-radius:12px 12px 0 0; display:flex; justify-content:space-between; align-items:center;">
...
<script>
    ...
    function viewDispatch(saleID) {
        ...
        setTimeout(() => {
            if (typeof window.viewDispatch === 'function') {
                window.viewDispatch(saleID);
            }
        }, 500);
    }
