...
// Fetch DO sales approved by admin but awaiting superadmin approval (show region for superadmin)
$query = "
    SELECT ds.*, do.DO_Name, do.CNIC, ds.regionID, r.regionName, COUNT(dsi.itemID) as itemCount,
           COALESCE((
    ...
    LEFT JOIN regions r ON ds.regionID = r.regionID
    LEFT JOIN users ua ON ds.approvedByAdmin = ua.user_id
    WHERE ds.approvedByAdmin IS NOT NULL AND ds.approvedBySuperAdmin IS NULL
    GROUP BY ds.saleID
...
<div class="approval-container">
    <div class="approval-header">
        <h2 style="margin:0">🔔 Dispatches — Awaiting Superadmin Approval</h2>
        <p style="margin:6px 0 0 0; opacity:0.9">Approve or reject dispatches across regions</p>
    </div>
    ...
    <div class="approval-card">
        <table class="approval-table">
            ...
            <tbody>
                <?php if (empty($sales)): ?>
                    <tr><td colspan="10" style="padding:18px; text-align:center; color:#666">No dispatches awaiting superadmin approval</td></tr>
                <?php else: foreach ($sales as $s): ?>
                    <tr>
                        ...
                        <td><span class="badge-region"><?php echo htmlspecialchars($s['regionName'] ?: 'N/A'); ?></span></td>
                        <td><?php echo $s['itemCount']; ?> items</td>
                        <td><?php echo htmlspecialchars($s['approvedByAdminName'] ?: ('ID: ' . ($s['approvedByAdmin'] ?? 'N/A'))); ?></td>
                        ...
                        <td>
                            ...
                            <button class="btn-small btn-approve" onclick="approveSuperSale(<?php echo (int)$s['saleID']; ?>, this)">Approve</button>
...
function approveSuperSale(id, btn) {
    Swal.fire({ title: 'Approve this dispatch as Superadmin?', icon: 'question', showCancelButton: true }).then(r=>{ if(!r.isConfirmed) return; $(btn).prop('disabled', true).text('Approving...'); $.post('backend/approveDOSaleSuper.php',{ saleID: id }, function(resp){ if(resp && resp.success){ Swal.fire('Approved','Dispatch approved by superadmin','success').then(()=> location.reload()); } else { Swal.fire('Error', resp && resp.message ? resp.message : 'Failed','error'); $(btn).prop('disabled', false).text('Approve'); } }, 'json').fail(()=>{ Swal.fire('Error','Server error','error'); $(btn).prop('disabled', false).text('Approve'); }); });
}
